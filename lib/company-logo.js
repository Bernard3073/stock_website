...
//      source we can actually promise. Covers ~70 widely held tickers; run
//      `npm run build-brand-marks` after editing that list.
//   2. Parqet's public logo assets, keyed by ticker. Proper brand marks, and it
//      needs no domain, so it works for tickers we have no profile for.
//   3. The site's apple-touch-icon via DuckDuckGo, keyed by domain. Modern
//      brands ship a 180px version of their mark here.
//   4. Google's favicon service at 128px, keyed by domain. Lowest fidelity but
