...
async function fetchFundamentals(symbol) {
  const modules =
    "incomeStatementHistory,incomeStatementHistoryQuarterly,earnings,earningsHistory,financialData,recommendationTrend,assetProfile,summaryProfile";
...
function mapFundamentals(payload) {
  ...
  const annualIncome = annualStmts.map(mapStatement).filter((s) => s.endDate);
  const quarterlyIncome = quarterlyStmts.map(mapStatement).filter((s) => s.endDate);
  ...
  const profileRaw = result.assetProfile || result.summaryProfile || {};
  const companyProfile = {
    ...
    fullTimeEmployees: getRaw(profileRaw.fullTimeEmployees),
    longBusinessSummary: profileRaw.longBusinessSummary || null,
    domain: toDomain(profileRaw.website)
  };
  ...
  return {
    ...
    companyProfile: hasCompanyProfile ? companyProfile : null
  };
}
...
function intervalForRange(range) {
  return INTERVAL_BY_RANGE[range] || "1d";
}
...
async function fetchNewsForSymbol(symbol, maxItems = 6) {
  try {
    const searchTerm = encodeURIComponent(`${symbol} stock`);
    const rss = await fetchText(
      `https://news.google.com/rss/search?q=${searchTerm}&hl=en-US&gl=US&ceid=US:en`
    );
    const itemMatches = [...rss.matchAll(/<item>([\s\S]*?)<\/item>/g)].slice(0, maxItems);
    ...
    return itemMatches
      .map((match) => {
        ...
        return {
          symbol,
          title: stripHtml(titleMatch?.[1] || `${symbol} market update`),
          link: decodeHtmlEntities(
            linkMatch?.[1] || `https://finance.yahoo.com/quote/${symbol}`
          ),
...
async function fetchNewsForStocks(stocks) {
  ...
  try {
    const newsGroups = await Promise.all(
      candidates.map((stock) => fetchNewsForSymbol(stock.symbol, 2))
    );
    // Interleave so the feed alternates symbols instead of grouping all AAPL together
    const maxLen = Math.max(...newsGroups.map((g) => g.length), 0);
    const interleaved = [];
    ...
    return interleaved.length ? interleaved : FALLBACK_NEWS;
  } catch {
...
export async function getStockQuoteWithHistory(symbol, range = "1mo", options = {}) {
  ...
  const chartReq = fetchChartBySymbol(match.symbol);
  const historyReq = fetchChartHistory(match.symbol, range);
  const fundamentalsReq = options.skipFundamentals
    ? Promise.resolve(null)
    : fetchFundamentals(match.symbol);
  const newsReq = options.skipNews
    ? Promise.resolve([])
    : fetchNewsForSymbol(match.symbol, 6);

  const [chartPayload, historyPayload, fundamentalsPayload, news] = await Promise.all([
    chartReq,
    historyReq,
    fundamentalsReq,
    newsReq
  ...
  return {
    ...
    news: options.skipNews ? undefined : news
  };
}
...
export async function getTrendingStocks() {
  ...
  try {
    ...
    const [details, news] = await Promise.all([
      fetchQuoteDetails(sortedStocks.map((s) => s.symbol)),
      fetchNewsForStocks(sortedStocks)
    ]);

    const rankedStocks = sortedStocks.map((stock) => ({
      ...stock,
      sector: details[stock.symbol]?.sector || null,
      industry: details[stock.symbol]?.industry || null,
      domain: details[stock.symbol]?.domain || null
    }));
...
async function fetchAssetProfile(symbol) {
  try {
    ...
    const url = `https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(
      symbol
    )}?modules=assetProfile,summaryProfile&crumb=${encodeURIComponent(crumb)}`;
    ...
    const cleanIndustry = (raw) => {
      if (!raw) return null;
      const trimmed = String(raw).trim();
      if (trimmed.length > 40) return trimmed.split(/[,;]/)[0].trim();
      return trimmed;
    };
    return {
      sector: profile.sector ? String(profile.sector).trim() : null,
      industry: cleanIndustry(profile.industry),
      website: profile.website || null,
      domain: toDomain(profile.website)
    };
  } catch {
    return null;
  }
...
async function fetchQuoteDetails(symbols) {
  ...
  const entries = await Promise.all(
    symbols.map(async (symbol) => [symbol, (await fetchAssetProfile(symbol)) || {}])
  );
